...
tags: [Pass through, End to End TLS, TLS Termination]
...
TLS 세션이 Client에서 시작되어 중간 장비에서 복호화하여 종료되는 방식을 TLS Termination이라고 합니다.
...
TLS 세션이 Client에서 시작되어 중간 장비에서 복호화된후, HTTP 트래픽을 검사하고 다시 TLS로 암호화하여 실제 Web Server로 전송하는 방식을 `TLS Bridging(Re-encryption)`이라고 합니다.
...
(본 예시는 Client -> L7 LB/WAF -> WEB Server인 경우를 가정합니다.)